/**
 * Ingest only the new Ovani products (data/ovani_new_products.json) into DB + embeddings.
 * Usage: npx tsx scripts/ingest-ovani-new.ts
 */
import fs from "fs";
...
async function main() {
  const filePath = path.join(process.cwd(), "data", "ovani_new_products.json");
  if (!fs.existsSync(filePath)) {
    console.error("Missing data/ovani_new_products.json — run merge first.");
    process.exit(1);
  }
  ...
  console.log(`Ingesting ${products.length} new Ovani products...`);
