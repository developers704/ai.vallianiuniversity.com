/**
 * Merge data/shopify_new_products.json into data/processed_products.json
 * Usage: npx tsx scripts/merge-shopify-new-products.ts
 */
import fs from "fs";
...
const NEW_PATH = path.join(ROOT, "data", "shopify_new_products.json");
...
function main() {
  if (!fs.existsSync(NEW_PATH)) {
    console.error(`Missing ${NEW_PATH} — run: npx tsx scripts/fetch-shopify-new-products.ts`);
    process.exit(1);
  }
