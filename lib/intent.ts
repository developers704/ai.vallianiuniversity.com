...
const STORE_PATTERNS =
  /\b(store\s*hours|location|address|contact|phone\s*number|email\s*us|near\s*me|find\s*a\s*store|store\s*locator|mall|where\s*are\s*you\s*located)\b/i;
...
const CONVERSATIONAL_PATTERNS =
  /^(ok|okay|k|yes|yeah|yep|yup|sure|alright|all\s*right|got\s*it|sounds\s*good|sounds\s*great|perfect|great|cool|nice|good|fine|understood|thanks?|thank\s*you|thx|no\s*problem|will\s*do|that\s*works|got\s*cha)[!?.,\s]*$/i;
...
export function isConversationalAcknowledgment(message: string): boolean {
  return CONVERSATIONAL_PATTERNS.test(message.trim());
}

export function isFarewell(message: string): boolean {
  return FAREWELL_PATTERNS.test(message.trim());
}
...
export function classifyIntentRuleBased(message: string): {
  ...
  if (RECOMMEND_PATTERNS.test(msg)) {
    return { intent: "PRODUCT_RECOMMENDATION", confidence: 0.8 };
  }
